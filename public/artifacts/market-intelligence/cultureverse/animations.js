...
const io = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    ...
    // Decision criteria bars
    if (el.classList.contains('crit-bars')) {
      el.classList.add('vis');
      el.querySelectorAll('.cb-fill').forEach((bar, i) => {
        setTimeout(() => { bar.style.width = bar.dataset.w + '%'; }, i * 100 + 200);
      });
    }

    // Revenue mix bars
    if (el.classList.contains('rev-bars')) {
      el.querySelectorAll('.rev-bar-fill').forEach((bar, i) => {
        setTimeout(() => { bar.style.width = bar.dataset.w + '%'; }, i * 100 + 200);
      });
    }
...
function observeAll() {
  document.querySelectorAll('.rv, .stag, #chain1, #stats1, #bars1, #donuts1, .tbl-rv, .crit-bars, h2').forEach(el => io.observe(el));
}
